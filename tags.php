...
if (isset($_POST["TagDelSub"])) delTag($db, $_POST["tagDelId"]);//WHY DOESN'T ALTER TABLE tagConnections ADD CONSTRAINT foreing_tag FOREIGN KEY(tagId) REFERENCES tags(tagId) ON DELETE CASCADE WORK, THERE'S NOTHING WRONG!!!!
...
-->
        <form action="" method="POST" class="TagDelForm">
            <input type="hidden" value="<?=$tag["tagId"]?>" name="tagDelId">
            <input type="submit" value="delete tag" name="TagDelSub" class="TagDelSub">
        </form>
